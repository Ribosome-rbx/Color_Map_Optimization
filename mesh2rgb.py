...
def main():
    ...
    data_root = '../AnnaTrain'
    mesh = o3d.io.read_triangle_mesh('./outputs/room_cmop.ply')
    # pcd = o3d.io.read_point_cloud('./outputs/full_scene.pcd')
    total_frames = len(listdir(data_root+"/Video")) - 4
    test_list = [0,1,2,3]
    # import glob
    # image_paths = glob.glob("E:/3DVision/Deblur-NeRF/datasets/img/images/*.png")
    # test_list = []
    # for path in image_paths:
    #     test_list.append(int(path.split("\\")[-1][:-4]))

    for frame_num in test_list:
        # get camera pose of i_th RGB frame
        intrinsic, extrinsic = get_camera_pose(data_root, frame_num) 
        ...
        vis.capture_screen_image(rgb_store + "/%06d" %frame_num + ".png")
